...
.DESCRIPTION
    ...
    The script supports reading additional options from a scan.config file.
...
# Read options from scan.config if it exists
$optsFile = "scan.config"
$transOptions = ""
$scanOptions = ""
$configSSCUrl = ""
$configSSCAuthToken = ""
...
if (($UploadToSSC -and -not $AviatorAuditOnly) -or $SSCUploadOnly) {
    ...
    if ([string]::IsNullOrEmpty($SSCUrl) -or [string]::IsNullOrEmpty($SSCAuthToken) -or 
        ...
        Write-Error "SSC configuration incomplete. Please ensure all required values are provided via parameters, environment variables (SSC_URL, SSC_AUTH_TOKEN, SSC_APP_NAME, SSC_APP_VERSION_NAME), or scan.config file [ssc] section"
        exit 1
    }
...
if (($AuditWithAviator -or $AviatorAuditOnly) -and -not $SSCUploadOnly) {
    ...
    if ([string]::IsNullOrEmpty($SSCUrl) -or [string]::IsNullOrEmpty($SSCAuthToken) -or 
        ...
        Write-Error "Aviator audit configuration incomplete. Please ensure SSCUrl, SSCAuthToken, AviatorUrl, AviatorToken, AviatorAppName, AppName, and AppVersion are all configured via parameters, environment variables, or scan.config file"
        exit 1
    }
